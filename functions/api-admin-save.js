...
async function sendTelegram(context, text) {
  ...
  // If not configured, just skip quietly (no error)
  ...
  // Optional: if you want to debug telegram errors in cloudflare logs
  // const data = await resp.json().catch(() => ({}));
  // if (!resp.ok || !data.ok) console.log("Telegram error:", data);
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // ---- Telegram (send only after success) ----
    ...
    // Don’t break saving even if Telegram fails
    try {
      await sendTelegram(context, msg);
    } catch (e) {
      console.log("Telegram send failed:", e?.message || String(e));
    }
